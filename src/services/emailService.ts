...
function getApiKey(): string {
  ...
  if (!key) {
    log.warn('emailService: BREVO_API_KEY not set — emails will not be sent');
  }
  return key;
}
...
async function sendEmail(payload: BrevoEmailPayload): Promise<void> {
  ...
  if (!apiKey) return; // Skip silently if no key — don't crash the server
